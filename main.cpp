...
std::string standard_message(){
    ...
    if(count == 0){
        ...
        switch (randomNumber) {
            case 0:
                msg = "Client says very Good!\n";
                break;
            case 1:
                msg = "Client says excellent!\n";
                break;
            case 2:
                msg = "Client says nice work!\n";
                break;
            case 3:
                msg = "Client says keep up the good work!\n";
                break;
            default:
                msg = "Client says You are doing great!!\n";
        }
    }
    return msg;
}
...
int main() {
    ...
    while(strcmp(terminate_msg,"EXIT!")!=0){
        ...
        if(!error) {
            std::cout << "Client sent message: " << client_message << std::endl;
        }
        else {
            std::cout << "Client message sent failure: " << error.message() << std::endl;
        }
        // receive data
        boost::asio::streambuf receive_buffer;
        boost::asio::read(socket, receive_buffer, boost::asio::transfer_all(), error);
        if( error && error != boost::asio::error::eof ) {
            std::cout << "Client receive message failed: " << error.message() << std::endl;
        }
        else {
